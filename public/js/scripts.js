...
function gamestable(loc, parameters, callback) {
    ...
    games.done( function(data) {
        ...
        else
        {
            ...
            if (parameters.hasOwnProperty('house'))
            {
                var d = new Date("24:00:00");
                var gamedata = "<caption><h4>Click on a game for more info, or right-click a game to change your attendance status.</h4><br></caption>";
                gamedata += "<thead><tr><th class='sort' data-sort='sport'>Sport</th><th class='sort' data-sort='opponent'>Opponent</th><th class='sort' data-sort='date_sec'>Date</th><th class='sort' data-sort='time_sec'>Time</th>";
                gamedata += "<th class='sort' data-sort='filterlocation'>Location</th><th class='sort' data-sort='filterattending'>Attending?</th></tr></thead><tbody class='list'>";
                // for loop through each message
                for (var i = 0, length = data.length; i < length; i++)
                {

                    gamedata += "<tr class='home' value = " + data[i].gameid + " att = " + data[i].attending;
                    gamedata += " url='/game_page.php?gameid=" + data[i].gameid + "'><td class='sport'>" + data[i].sport + "</td>";
                    gamedata += "<td class='opponent'>" + data[i].opponent + "</td><td class='filterdate'>" + data[i].date + "</td><td class='date_sec' style='display: none'>" + (-Date.parse(data[i].date)) + "</td><td class='filtertime'>" + data[i].timepretty;
                    gamedata += "</td><td class='time_sec' style='display: none'>" + data[i].time + "</td><td class='filterlocation'>" + data[i].location + "</td><td class='filterattending'>" + data[i].attending + "</td></tr>";
                }
                gamedata += "</tbody>";
...
function loadroster(roster, parameters) {
    ...
    players.done(function(data) {
        ...
        {
            if (parameters.hasOwnProperty('house')) 
                rosterdata = "<h5>No one signed up from " + parameters.house + "</h5>";
            else
        ...
        else
        {
            ...
            rosterdata += "<table class='table' style='border:1px solid gray;'>";
            ...
            for (var i = 0, length = data.length; i < length; i++)
            {
                ...
                rosterdata += "<tr class='home' style='height: 67px;' url='/profile.php?id=" + data[i].userid + "''>" +
                            "<td style='vertical-align: middle; width:30%;'>" + (i+1) + ".</td>" + 
                            "<td style='vertical-align: middle; font-weight: bold; width: 40%;'>" + data[i].name + "</td>" + 
                            "<td style='width: 30%;'><a href='/profile.php?id=" + data[i].userid + "'><img src = '" + smallPic + "' style='border-radius: 25px; float:left;'/></a></td>" + 
                            "</tr>";
            }
            rosterdata += "</table>";
        }
...
function rosterupdate(parameters, number) {
    $.ajax({
        data: {
    ...
    loadroster(document.getElementById(number), parameters);
}
...
// add a roster to a game page
function refloadroster(roster, parameters) {
    var players = $.getJSON("search_rosters.php", parameters)
    players.fail(function() {
        alert("Loading roster failed.");
    }); 
...
// add user to game roster
function refrosterupdate(parameters) {
    $.ajax({
        data: {
            gameid: parameters.gameid,
            attending: parameters.attending
    ...
    refloadroster(document.getElementById('roster'), parameters);
}
